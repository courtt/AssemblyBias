...
def clustering(root_sims,folder,count,dims1,dims,bins):#ptype
    ...
    list1 = [ x for x in range(len(index))]
    #print list1
    np.random.shuffle(list1)
    print list1[0:10]
    half = np.int_(len(list1)/2)
    #print half
    indexAbove = list1[0:half]
    #print indexAbove
    indexBelow = list1[half:]
    print len(indexAbove)
    ...
    #indexAbove.append(indexBelow)
    #np.random.shuffle(indexAbove)
...
for root_folder in ['0.0eV/','0.6eV/']:
    ...
    for dims1 in [64,128,256,512,1024]:
        print dims1
        folder = root_output+'random_%s_%i/'%(root_folder[:-1],dims1)
        #for ptype in [1,2]:
         #   print ptype
