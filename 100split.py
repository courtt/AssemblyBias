...
def clustering(root_sims,folder,count,ptype,dims1,dims,bins):
    ...
    for i in xrange(bins):
        ...
        rho_median = np.median(rho[index2]) # index2 stores the indeces of what's in each bin
        rho_diff = rho[index2]- rho_median
...
################ COMPUTE DENSITY AND P(k) #############################
    ...
    np.savetxt(fout4, 
               np.transpose([Pk.k3D, Pk.Pk[:,0,0]+Pk.Pk[:,0,1]+2*Pk.XPk[:,0,0]]))
