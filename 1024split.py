...
for root_folder in ['0.0eV/','0.15eV/']:
    root_sims = root_total + root_folder + '1024/'
    print root_folder
    for dims1 in [64,128,256,512,1024]:
        ...
        for ptype in [1,2]:
            ...
            if ptype==1: folder = root_output+'1024results_cross_c_%s_%i/'%(root_folder[:-1],dims1)
            if ptype==2: folder = root_output+'1024results_cross_nu_%s_%i/'%(root_folder[:-1],dims1)
            

            # create folder if it does not exists
            if myrank==0:
                if not(os.path.exists(folder+'Pk_haa_16.txt')):  
